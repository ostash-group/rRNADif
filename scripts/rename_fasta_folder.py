...
def write_fasta():
    ...
    if check_if_are_entries():
        with open(new_path +'/' +names[0].replace(' ', '_').replace('/', '_').replace(':', '_').replace(';', '_').replace(',', '_').replace('[','').replace(']','')+".fasta", "w") as f:
            for i in range(len(seqs)):
                f.write(">" + names[0].replace(' ', '_').replace('/', '_').replace(':', '_').replace(';', '_').replace(',', '_').replace('[','').replace(']','') + "\n" +seqs[i] + "\n")
